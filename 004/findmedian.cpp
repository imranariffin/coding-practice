...
inline boolean 
medianOfTwoSortedArrays_m2_inner( const _Type* a, long a_left, long a_right, const _Type* b, long b_left, long b_right,
                                                 long num_elements_larger_than_median_in_total, long* median )
{
    ...
    while( low <= high )                 // termination means no overall median within A
    {
        ...
        {
            if ( a[ overall_median ] <= b[ b_left ] && i_larger_in_b == b_left )
            // satisfies being an overall median. check the element to the right (check #1)
            {
                *median = overall_median;
                return true; // median was found in A
            }
            else {  
            	// a[ overall_median ] > b[ i_larger_in_b = 0 ] => guess of overal 
            	// median is too big to be the overall median. Need to move overall_median within A to become smaller
...
inline void medianOfTwoSortedArrays_m2( const _Type* a, long a_left, long a_right, const _Type* b, long b_left, long b_right, long* which_array, long* median )
{
    ...
    *median      = -1;      // by default set return median index to be invalid
    ...
    // high element in total minus median index
    // There may be a way to eliminate this special case of handling either one or both input arrays of length 0
    long num_elements_larger_than_median_in_total = ( total_length - 1 ) - median_in_total;
